...
class BuildingGenerator:
    ...
    def _generate_hill_terrain(self,screen_size: Tuple[int, int]):
        ...
        base_percentile = 30
        delta = 3
        buildings_ranges = ([range(base_percentile + (i * delta), base_percentile + ((i + 1) * delta)) for i in range(num_buildings + 1)])

        tallest_building_index = random.randint(0, num_buildings)
        left_buildings,right_buildings = building_range[0: tallest_building_index], building_range[tallest_building_index + 1:]
